        ...
        return __generator(this, function (_a) {
            fs.readdir('./', function (err, files) {
                if (err) {
                    ...
                    (0, fileChecks_1.readmeChecks)(files);
                    //Check for CODEOWNERS file in .github folder
                    (0, fileChecks_1.codeOwnerCheck)();
                    //Check if nodemodules folder is present in master branch for typescript action
                    (0, fileChecks_1.nodeModulesCheck)();
                    //check for branch permissions in main/master and releases/*
                    (0, branchPermissionCheck_1.branchPermissionCheck)(repository, ownername, secret_token, octokit);
                    //check for nodemodules folder in releases/*
                    (0, fileChecks_1.releasesNodeModulesCheck)(repository, ownername, secret_token, octokit);
                    //check for security/vulnerability bot
                    (0, vulnerabilityBotCheck_1.vulnerabilityBotCheck)(repository, ownername, secret_token, octokit);
                    //1. check whether issue-template has been set up and 2. default label is need-to-triage
                    (0, issueTemplateCheck_1.issueTemplateCheck)();
                    //Check whether standard labels have been set up
                    (0, standardLabelsCheck_1.standardLabelsCheck)(repository, ownername, secret_token, octokit);
                }
            });
            return [2 /*return*/];
        });
